...
public class PacientesRepository {
    ...
    private final ListaDoblementeEnlazada<Paciente> listaPacientes; //Lista enlazada que almacena los objetos de tipo Paciente, en esta caso usamos una lista
    ...
    public ListaDoblementeEnlazada<Paciente> obtenerTodos() {//si funciona
    ...
    public void añadirPacientes(ListaDoblementeEnlazada<Paciente> nuevosPacientes) {
    ...
    public Paciente obtenerPorId(String idPaciente) {
        ListaDoblementeEnlazada<Paciente> pacientes = obtenerTodos(); // Obtener la lista completa de pacientes
        for (int i = 0; i < pacientes.tamano(); i++) { // Recorrer la lista de pacientes
            Paciente paciente = pacientes.obtenerElementoEnPosicion(i); // Obtener el paciente en el índice actual
