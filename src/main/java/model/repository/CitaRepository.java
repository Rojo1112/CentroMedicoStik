...
public class CitaRepository {
    ...
    private final ListaDoblementeEnlazada<Cita> citasHistorial;

    /**
     * Constructor de la clase RegistroExamenRepository
     */
    public CitaRepository() {
        this.pathFile = "src/main/java/dataBase/citasHistorial.Json"; // Ruta del archivo JSON dentro del proyecto
        this.jsonAdapterCita = FileJsonAdapter.getInstance(); // Obtener una instancia del adaptador JSON para Pacientes
        this.citasHistorial = jsonAdapterCita.getObjects(pathFile, Cita[].class);
    }
    ...
    public void guardarCita(Cita cita) {
        citasHistorial.agregarAlFinal(cita); // Agregar el nuevo registro de examen a la lista de registros
        jsonAdapterCita.writeObjects(pathFile, citasHistorial); // Guardar la lista de registros en el archivo JSON
    }

    //Metodo eliminar cita
    public void eliminarCita(Cita cita) {
        // Iterar sobre la lista de pacientes para buscar el paciente a eliminar
        int indice = citasHistorial.buscarElemento(cita); // Busca el índice del paciente en la lista
        if (indice != -1) { // Si se encuentra el paciente en la lista
            citasHistorial.eliminarEnMedio(indice); // Elimina el paciente de la lista
    ...
    public ListaDoblementeEnlazada<Cita> buscarCitasPorIdPaciente(String idPaciente) {
        ListaDoblementeEnlazada<Cita> citasPaciente = new ListaDoblementeEnlazada<>();

        int indice = 0; // Inicializamos el índice en 0 para el primer nodo
        int tamano = citasHistorial.tamano(); // Obtenemos el tamaño de la lista

        while (indice < tamano) { // Mientras el índice sea menor que el tamaño de la lista
            Cita cita = citasHistorial.obtenerElementoEnPosicion(indice); // Obtener el registro en el índice actual
    ...
    public ListaDoblementeEnlazada<Cita> obtenerTodos() {
        return citasHistorial; // Retorna la lista de pacientes
    }
}