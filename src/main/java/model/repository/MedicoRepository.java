...
public class MedicoRepository {
    ...
    private final ListaDoblementeEnlazada<Medico> listaMedicos; //Lista enlazada que almacena los objetos de tipo Paciente, en esta caso usamos una lista
    ...
    public ListaDoblementeEnlazada<Medico> obtenerTodos() {//si funciona
    ...
    public Medico obtenerPorId(String idMedico) {
        ListaDoblementeEnlazada<Medico> medicos = obtenerTodos(); // Obtener la lista completa de pacientes
        for (int i = 0; i < medicos.tamano(); i++) { // Recorrer la lista de pacientes
            Medico medico = medicos.obtenerElementoEnPosicion(i); // Obtener el paciente en el índice actual
    ...
    public ListaDoblementeEnlazada<Medico> buscarPorEspecialidad(String especialidad) {
        ListaDoblementeEnlazada<Medico> medicos = obtenerTodos(); // Obtener la lista completa de pacientes
        ListaDoblementeEnlazada<Medico> medicosEspecialidad = new ListaDoblementeEnlazada<>();
        for (int i = 0; i < medicos.tamano(); i++) { // Recorrer la lista de pacientes
            Medico medico = medicos.obtenerElementoEnPosicion(i); // Obtener el paciente en el índice actual
